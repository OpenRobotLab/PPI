...
class DiffusionHeadPointflowContinuous(nn.Module):
# ablation for predict continuous actions conditioned on pointflow
    def __init__(self,
                 embedding_dim=120                                                                                                                               ,
                 num_attn_heads=8,
    ...
    def prediction_head(self,
        ...
        # Cross attention from pointflow features to full pcd
        ...
        # 1. point flow prediction        
        features_point_flow = torch.cat([point_flow_features, sampled_pcd_features], 0)
        rel_pos_point_flow = torch.cat([rel_initial_point_flow, sampled_rel_pcd_pos], 1)
        ...
        # detach pointflow features, which act as conditions for continuous action
        features_point_flow_detach = features_point_flow.detach()
        rel_pos_point_flow_detach = rel_pos_point_flow.detach()
        features_continuous = torch.cat([gripper_features_left[continuous_start:keyframe_start,:,:], gripper_features_right[continuous_start:keyframe_start,:,:],features_point_flow_detach], 0)
        rel_pos_continuous = torch.cat([rel_gripper_pos_left[:,continuous_start:keyframe_start,:,:], rel_gripper_pos_right[:,continuous_start:keyframe_start,:,:], rel_pos_point_flow_detach], 1)
        ...
        position_left = torch.cat((position_left_continuous, position_left_continuous[:,:self.horizon_keyframe,:]), 1)
        position_right = torch.cat((position_right_continuous, position_right_continuous[:,:self.horizon_keyframe,:]), 1)
        rotation_left = torch.cat((rotation_left_continuous, rotation_left_continuous[:,:self.horizon_keyframe,:]), 1)
        rotation_right = torch.cat((rotation_right_continuous, rotation_right_continuous[:,:self.horizon_keyframe,:]), 1)
        openess_left = torch.cat((openess_left_continuous, openess_left_continuous[:,:self.horizon_keyframe,:]), 1)
        openess_right = torch.cat((openess_right_continuous, openess_right_continuous[:,:self.horizon_keyframe,:]), 1)
