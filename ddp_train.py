        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            if (self.epoch % cfg.training.sample_every) == 0:
                with torch.no_grad():
                    ...
                    if cfg.policy.predict_point_flow:
                        if cfg.policy.what_condition!="pointflow_continuous":
                            # set_trace()
                            ...
                            if self.local_rank==0:
                                ...
                                step_log['point_flow_pos_acc_001'] = (pos_l2_point_flow < 0.01).float().mean()
                        else:
                            gt_point_flow = batch['point_flow'][:,-cfg.horizon_keyframe:,:,:]
                            b, k, N, _ = gt_point_flow.shape
                            gt_point_flow = gt_point_flow.reshape(b,k*N,3)
                            ...
                            mse = torch.nn.functional.mse_loss(pred_action, gt_action)
                            mse_point_flow = torch.nn.functional.mse_loss(pred_point_flow, gt_point_flow)
                            left_pos_l2 = ((pred_action[:,:cfg.horizon_continuous, :3] - gt_action[:,:cfg.horizon_continuous, :3]) ** 2).sum(-1).sqrt()
                            left_quat_l1 = (pred_action[:,:cfg.horizon_continuous, 3:7] - gt_action[:,:cfg.horizon_continuous, 3:7]).abs().sum(-1)
                            right_pos_l2 = ((pred_action[:,:cfg.horizon_continuous, 7:10] - gt_action[:,:cfg.horizon_continuous, 7:10]) ** 2).sum(-1).sqrt()
                            right_quat_l1 = (pred_action[:,:cfg.horizon_continuous, 10:14] - gt_action[:,:cfg.horizon_continuous, 10:14]).abs().sum(-1)
                            pos_l2_point_flow = ((pred_point_flow[..., :3] - gt_point_flow[..., :3]) ** 2).sum(-1).sqrt()
                            if self.local_rank==0:
                                step_log['train_action_mse_error'] = mse.item()
                                step_log['train_point_flow_mse_error'] = mse_point_flow.item()
